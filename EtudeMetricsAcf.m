...
%
%   INPUT
%     r   : autocorrelation (column or row), assumed lags >= 0 (r(1) = lag 0)
%     Opt : (optional) struct with fields:
%           .MinPromFrac      (default 0.02)  % prominence threshold as frac of max(r)
%           .MinPeakDistance  (default 2)     % in samples, to avoid adjacent tiny peaks
%           .NPeaksForStats   (default 8)     % number of first peaks to use for stats
%           .PersistenceFrac  (default 0.2)   % peaks above this*pk1 are "persistent"
%           .Weight           (struct) weights for final score:
%               .Regularity    (default 0.30)
%               .P2B           (default 0.30)
%               .DecayR2       (default 0.15)
%               .PromQuality   (default 0.10)
%               .ZCPerInt      (default 0.10)
%               .Anti2T        (default 0.30) % used as a *penalty*
%
%   OUTPUT (struct M)
%     M.pk1, M.pk2, M.lag1, M.lag2
%     M.intervals_mean, M.intervals_std, M.regularity  (1/(1+CV))
%     M.zero_crossings, M.zc_rate, M.zc_per_interval
%     M.p2b           : peak-to-background ratio (pk1 / background RMS)
%     M.prom_mean, M.prom_std, M.prom_quality
%     M.decay_R2      : R^2 of exponential decay of peak heights
%     M.altPeaksScore : |mean(odd) - mean(even)| / mean(all peaks)   (2T detector)
%     M.first_second_ratio : pk2/pk1
%     M.persistence_count  : #peaks >= PersistenceFrac*pk1
%     M.period_est_samples : mean inter-peak distance (rounded)
%     M.Score         : final weighted score (higher is better)
%     M.Flags.Suspect2T : boolean flag if 2T suspected
%
%   Notes:
%     - Conçu pour comparer la “qualité ACF” en TEMPS uniquement.
