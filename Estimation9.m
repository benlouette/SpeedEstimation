...
% file = "D:\DATAS\Data_DGBB_for_speed\P70 AA12 01R Acc";
% file = "D:\DATAS\Data_DGBB_for_speed\PDA A9 02V Acc";
% file = "D:\DATAS\Data_DGBB_for_speed\PTF1 02R Acc";
% file = "D:\DATAS\Data_DGBB_for_speed\PTF1 03R Acc";
% fid = fopen(file+".csv");
% array = cell2mat(textscan(fid, '%f', 'Delimiter',',')); % you will need to change the number   of values to match your file %f for numbers and %s for strings.
% fclose (fid);
% FsOr = 25600;

% % Nom du fichier (à adapter)
% filename = 'C:/Users/GJ2640/Downloads/archive (1)/1st_test/1st_test/2003.10.22.12.09.13';
% % Read as text file with space delimiter
% opts = detectImportOptions(filename,'FileType','text');
% opts.Delimiter = ' ';   % force space delimiter
% 
% data = readmatrix(filename, opts);
% 
% disp(size(data));   % should be [20479 4]

% 1) Si tu as déjà extrait les fichiers localement :
db = buildIMSDatabase("C:\Users\GJ2640\Bureau\DataMachine\IMSBearingData\3rd_test\4th_test\txt", "C:\Users\GJ2640\Bureau\DataMachine\IMS_DB");

% 2) Ou depuis une URL ZIP (remplace par l’URL réelle) :
% db = buildIMSDatabase("https://.../IMS-Rexnord%20Bearing%20Data.zip", "C:\IMS_DB");

% Charger l'index
load("C:\IMS_DB\ims_index.mat","db");
head(db.index)

% Ouvrir un enregistrement et le tracer
row = db.index(1,:);
S = load(fullfile("C:\IMS_DB", row.RelMatPath));  % data, FsLoc, Ts, NLoc, NCh
t = (0:S.NLoc-1).' / S.FsLoc;
plot(t, S.data(:,1)); grid on
title(sprintf('Set %d | %s | Ch1', row.Set, string(S.Ts)))
xlabel('Temps (s)'); ylabel('Acc (g ? unités d''entrée)');
...
%     dec = 5;
%     array = decimate(array, dec, 1, 'fir');
%     FsOr = FsOr /dec;
% 
% test = load('C:\Users\GJ2640\Downloads\archive (3)\raw\Time_Normal_1_098.mat');
% FsOr=48000;
% arrayA=test.X098_DE_time;
% arrayB=test.X098_FE_time;
% sound(array2B,FsOr);
%%
% 
% % Définir le dossier contenant les données
% data_folder = fullfile('D:/DATAS/DATA_SET/NASADataSet/');
% dataset_folder_prefix = '2nd';
% dataset_folder = fullfile(data_folder, sprintf('%s_test', dataset_folder_prefix));
% 
% % Lister tous les fichiers dans le dossier
% data_files = dir(fullfile(dataset_folder, '*')); % ou *.txt selon l'extension
% 
...
%     
%     % Lire le fichier
%     d = readtable(file_path, 'Delimiter', '\t', 'ReadVariableNames', false);
%     
%     % Si une seule colonne, essayer de splitter
%     if width(d) == 1
%         d = split(d.Var1, '\t');
%     end
%     
%     % Renommer les colonnes
%     d.Properties.VariableNames = {'Bearing1', 'Bearing2', 'Bearing3', 'Bearing4'};
%     
%     % Sélectionner les premières 20480 lignes
%     num_rows = height(d);
%     selected_data = d(1:min(20480, num_rows), :);
%     
%     % Extraire le timestamp depuis le nom du fichier
%     [~, timestamp_str, ~] = fileparts(data_files(k).name);
%     timeatemp = strrep(timestamp_str, '_', ':');
%     
%     try
%         timestamp = datetime(timestamp_str, 'InputFormat', 'yyyy_MM_dd_HH_mm_ss');
%     catch
%         timestamp = NaT; % Not a Time si erreur
%     end
%     
%     % Affichage pour vérification
%     fprintf('Fichier: %s | Timestamp: %s\n', data_files(k).name, datestr(timestamp));
% test5 = load('C:\Users\GJ2640\Downloads\archive (4)\data\data-20130421T151231Z.mat'); % 1725 Rpm
% array5 = test5.vibration;
% FsOr = numel(array5) / 6.0;
% freqRot5 = 60.88 ;

% array   =   array1A;
% freqRot = freqRot1 ;

sizeSlide=min(8192*2,numel(array));
n=floor(numel(array)/sizeSlide);

% --- Exemple d’appel ---
fc_list = 600:100:3000;               % Hz
Opt = struct();
Opt.sweep.envLP_Hz   = 200;                 % coupe LP de l’enveloppe (>= 2*fmax rotation)
Opt.sweep.acfMaxSec  = 1.0;                 % ACF jusqu’à 1 s
Opt.sweep.scoreW     = [0.4 0.4 0.2];       % [w_acf, w_spec, w_ent]
Opt.sweep.smoothMs   = 1.0;                 % léger lissage ACF avant métriques
Opt.sweep.nfftACF    = [];                  % auto
Opt.pick.maxPeaks = 12;
Opt.pick.minPromDB = 6;
Opt.pick.DeltaHz = [5 150];
Opt.pick.K = 3;
%     % x, Fs doivent exister dans ton workspace
accOrigi=array(1:sizeSlide);
RES = evaluate_carrier_sweep(accOrigi, FsOr, fc_list, Opt);
Opt.pick.rangeHz = [500 3000];
BEST = pick_best_carrier_fast(accOrigi, FsOr, Opt.pick);

scores=zeros(numel(fc_list),1);

for f = 1:numel(fc_list)
    result = defineBestRange(accOrigi,FsOr,fc_list(f),2);
    scores(f) = result;
end
[~,idx]=max(scores);
bestfc = fc_list(idx);
figure;plot(fc_list,scores);
plotFFT(accOrigi,FsOr);
xline(RES.fc_best,'g:');
xline(BEST.band(1),'m:');
xline(bestfc,'m:');
bestfc = 900;

%     BEST = robust_best_carrier(accOrigi, FsOr, fc_list, Opt);
    
%     fractal = zeros(numel(fc_list),1);
%     for i = 1:numel(fc_list)
%         fc = fc_list(i);
%         signalEnvelope = getEnv(accOrigi, FsOr, fc);
%         [AcfIn] = nsdf_mcleod_fast(signalEnvelope, FsOr, 0, 160, numel(signalEnvelope));
%         fractal(i) = absFeatures_FractalDatas(numel(AcfIn),AcfIn);
%     end
%     [~,idxmin]=min(fractal);
%     bestRange=fc_list(idxmin);
...
for slide_idx = 1:n
    ...
    signalEnvelope = getEnv(acc, Fs,bestfc );% RES.fc_best);% BEST.band(1)-50);  %   extracts the envelope
...
[OUT, Lmed,L0] = period_from_acf_median_robust(AcfIn, Fs, tauMinSamp, tauMaxSamp, Opt);
candf_mediane = OUT.f_hat;
    ...
    fcands = [candf_Cepstrum,candf_yin, candf_comb, candf_hps,candf_mediane,BEST.Delta];
...
%         'LabelOrientation','horizontal','LabelVerticalAlignment','bottom');
    ...
    subplot(3,1,3);
    vals = [freqRot,candf_Cepstrum,candf_yin, candf_comb, candf_hps, f_twm, candf_mediane,BEST.Delta];
...
% ScoreG(ScoreG == -Inf) = NaN;
% ScoreG=ScoreG-median(ScoreG,'omitnan');
% ScoreG = normalize_0_1(ScoreG);
% ScoreG(isnan(ScoreG))=0;
% f0_grid = linspace(50, 1200, 3000);
% H = harmonic_sum_spectrum(ScoreG, Fs, tauGrid, 10, '1/k', true);
% [~, idx] = max(H);
% f0_hat = tauGrid(idx)
...
% ScoreG = normalize_0_1(ScoreG);
%     %% Estimation de période par médiane des différences successives
%     [pkVal, pkLoc] = findpeaks(ScoreG, 'MinPeakProminence', 0.5, 'MinPeakDistance', 10);   % pkLoc = lags, pkVal = amplitudes
%     meandiff= median(diff(pkLoc));
%     
%     
% 
% 
...
function [OUT ,L_med,L0] = period_from_acf_median_robust(acf, Fs, tauMinSamp, tauMaxSamp, Opt)
...
function result = defineBestRange(accOrigi,FsOr,f_cut,dec)
    
    acc = decimate(accOrigi, dec, 1, 'fir');
    Fs = FsOr /dec;
    
    signalEnvelope = getEnv(acc, Fs,f_cut );% RES.fc_best);% BEST.band(1)-50);  %   extracts the envelope
    [AcfIn] = nsdf_mcleod_fast(signalEnvelope, Fs, 0, 160, numel(signalEnvelope));
...
candf_mediane = OUT.f_hat;
    ...
    result = score_qualite_estimateurs(fcands)
end
function score_qualite = score_qualite_estimateurs(fcands)
% fcands : vecteur contenant les estimateurs de fréquence
% score_qualite : score unique entre 0 (très mauvais) et 1 (très bon)

    % S'assurer que fcands est une ligne
    fcands = fcands(:)';
    
    % Étape 1 : estimation centrale (médiane)
    f_ref = median(fcands);
    
    % Étape 2 : calcul des ratios
    ratios = fcands / f_ref;
    
    % Étape 3 : détection des harmoniques (proches de 0.5, 1, 2)
    harmonic_candidates = [0.5, 1, 2];
    tol = 0.1;
    is_harmonic = false(size(fcands));
    
    for h = harmonic_candidates
        is_harmonic = is_harmonic | abs(ratios - h) < tol;
    end
    
    % Étape 4 : score de cohérence (écart à la médiane)
    score = abs(fcands - f_ref);
    
    % Étape 5 : score global
    % - Cohérence : inverse de la dispersion
    % - Harmoniques : proportion d'estimateurs valides
    dispersion = iqr(fcands);  % ou std(fcands)
    proportion_harmoniques = sum(is_harmonic) / length(fcands);
    
    % Normalisation du score de dispersion (plus petit = meilleur)
    score_dispersion = exp(-dispersion / f_ref);  % entre 0 et 1
    
    % Score final : moyenne pondérée
    score_qualite = 0.6 * score_dispersion + 0.4 * proportion_harmoniques;
end
...
function [freqs, mag] = mag_spectrum(x, Fs, useHann)
%MAG_SPECTRUM  Spectre d'amplitude simple-face (0..Fs/2).
%   [freqs, mag] = mag_spectrum(x, Fs, useHann)
%   - x        : signal (Nx1 ou 1xN)
%   - Fs       : fréquence d’échantillonnage (Hz)
%   - useHann  : true/false (fenêtrage Hann). Par défaut: true
%
%   mag : amplitude (corrigée simple-face). Pour la puissance, faire mag.^2.

    if nargin < 3, useHann = true; end
    x = x(:);                 % colonne
    N = size(x,1);

    % Fenêtre Hann sans toolbox (periodic)
    if useHann
        n = (0:N-1).';
        w = 0.5 - 0.5*cos(2*pi*n/N);   % périodique (N au dénominateur)
    else
        w = ones(N,1);
    end
    ...
    % Normalisation amplitude simple-face (~conservation d’amplitude)
    % Scale ~ 2/sum(w) ; corrige DC et Nyquist
    scale = 2 / sum(w);
    mag   = mag * scale;
    mag(1) = mag(1) / 2;
    if mod(N,2) == 0
...
function H = harmonic_sum_spectrum(x, Fs, f0_grid, K, weights, usePower)
%HARMONIC_SUM_SPECTRUM  Score par somme aux multiples (peigne harmonique).
%   H = harmonic_sum_spectrum(x, Fs, f0_grid, K, weights, usePower)
%   - f0_grid : vecteur des candidats f0 (Hz)
%   - autres paramètres comme sum_harmonics_known_f0
%
%   H(i) = somme pondérée aux multiples de f0_grid(i).

    if nargin < 4 || isempty(K), K = 8; end
    if nargin < 5 || isempty(weights), weights = '1/k'; end
    if nargin < 6, usePower = true; end

    [freqs, mag] = mag_spectrum(x, Fs, true);
    magv = mag; 
    if usePower
        magv = mag.^2;
    end

    H = zeros(size(f0_grid));
    for i = 1:numel(f0_grid)
        f0 = f0_grid(i);
        if f0 <= 0
            H(i) = 0;
            continue;
        ...
        if isa(weights, 'function_handle')
            w = arrayfun(weights, ks);
        else
            switch string(weights)
                case "1/k"
                    w = 1 ./ ks;
                case "1/sqrt(k)"
                    w = 1 ./ sqrt(ks);
                case "none"
                    w = ones(size(ks));
                otherwise
                    error('weights inconnu.');
            end
        end

        vals = interp1(freqs, magv, fks, 'linear', 0.0);
        H(i) = sum(w .* vals);
    end
end
