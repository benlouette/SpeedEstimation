...
% === DEMO OMA (IMS) ===
% data : (N×C) en double, C=4 ou 8
% fs   : 20000 Hz (d'après la doc IMS)
% % test = load('C:\Users\GJ2640\Bureau\DataMachine\IMS_DB\data\set1\2003.11.15.22.18.mat'); % 1500
% % 
% % A = test.data;
% % fs=test.FsLoc;
% % FsOr = fs;
...
% % 
% % % Paramètres OMA
% % mu   = 0.9990;      % facteur d'oubli (très proche de 1)
% % M    = 3;           % nb de modes suivis
% % Lfft = 4096;        % fenêtre FFT pour estimer f_n
% % D    = 1;           % décimation (>=1). Met 10 si tu veux aller très vite.
% % 
% % st = oma_init(size(A,2), fs, ...
% %               'ForgettingFactor', mu, ...
% %               'NumModes', M, ...
% %               'FftLen', Lfft, ...
% %               'Decim', D);
% % 
% % N = size(A,1);
% % f_hist = nan(floor(N/D), M);
% % t_hist = nan(floor(N/D), 1);
% % 
% % row = 0;
% % for n = 1:N
% %     [st, out] = oma_step(st, A(n,:).');
% %     if out.updated && mod(n, D)==0
% %         row = row + 1;
% %         f_hist(row,:) = out.freqs;
% %         t_hist(row)   = (n-1)/fs;
% %     end
% % end
% % 
% % % Nettoyage (enlève les lignes NaN en tête)
% % valid = ~isnan(t_hist);
% % t_hist = t_hist(valid);
% % f_hist = f_hist(valid,:);
% % 
% % % --- Plots rapides ---
% % figure; 
% % plot(t_hist, f_hist, 'LineWidth', 1.2); grid on
% % xlabel('Temps (s)'); ylabel('Fréquences propres estimées (Hz)');
% % title('Suivi des fréquences modales (OMA PCA+forgetting)');
% % legend(arrayfun(@(k) sprintf('Mode %d',k), 1:M, 'UniformOutput', false));
% % 
% % figure;
% % bar(st.varExpl); grid on
% % xlabel('Mode'); ylabel('Variance expliquée (%)');
% % title('Importance des modes courants');
% % 
% % % Derniers résultats
% % res = oma_finalize(st);
% % disp('Fréquences (Hz):'); disp(res.freqs(:).');
% % disp('Variance expliquée (%):'); disp(res.varExpl(:).');
sizeSlide=8192*2;
accOrigi=array(1:sizeSlide);
n=floor(numel(array)/sizeSlide);
fs = FsOr;
% st = sc_init(fs,'FftLen',4096,'Forgetting',0.9,'Decim',1);
% 
% % si tu as déjà un vecteur y (Nx1), boucle :
% f_hist = NaN(length(accOrigi),1);
% for n=1:length(accOrigi)
%     [st, o] = sc_step(st, accOrigi(n));
%     if o.updated
%         f_hist(n) = o.f_est;
%     end
% end
% 
% % trace (supprime les NaN initiaux)
% idx = ~isnan(f_hist);
% t = (find(idx)-1)/fs;
% plot(t, f_hist(idx)); grid on
% xlabel('Temps (s)'); ylabel('f dominante (Hz)');
% title('Suivi fréquence dominante (1 canal)');

% Initialisation du tracker ACF
% Tracker ACF
% st = sc_acf_init(fs,'WinLen',2048,'Forgetting',0.95,'Decim',1);
% x=accOrigi;
% f_est_hist = nan(length(x),1);
% 
% for n = 1:length(x)
%     [st,out] = sc_acf_step(st, x(n));
%     if out.updated
%         f_est_hist(n) = out.f_est;
%     end
% end
% 
% % Résultats valides
% idx = ~isnan(f_est_hist);
% t_est = (find(idx)-1)/fs;
% f_est = f_est_hist(idx);

% figure;
% plot(t_est, f_est, 'LineWidth',1.2);
% xlabel('Temps (s)');
% ylabel('Fréquence estimée (Hz)');
% title('IMS Bearing Data – suivi fréquence dominante (1 canal)');
% grid on;



% --- Exemple d’appel ---
fc_list = 100:100:10000;               % Hz
Opt = struct();
...
xline(BEST.band(1),'g:');
xline(BEST.band(1)*2,'m:');

Opt = struct('rangeHz',[100 2000], 'welchWin',4096, 'welchOL',0.5, 'nfft',16384, ...
             'maxPeaks',20, 'minPromDB',6, 'minDistHz',10, ...
             'K',3, 'DeltaHz',[5 80], 'etaBox',0.15, 'thrSNR_dB',3, ...
             'useMedianBaseline',true, 'Delta0_small',10);

res = pick_best_carrier_comb(accOrigi, FsOr, Opt);   % x: signal colonne
plot_pick_best_comb(res, Opt, 10);

% Résultat de ton picker (pick_best_carrier_comb)
% res = pick_best_carrier_comb(x, Fs, OptPicker);

% a) Version rapide FFT/IFFT (offline)
[S_fft, info_fft] = split_top_candidates(accOrigi, FsOr, res, ...
    'NumSignals',10, 'Method','fft', 'WidthFactor',3, 'K',3);

% b) Version filtres FIR (zéro-phase)
[S_fir, info_fir] = split_top_candidates(accOrigi, FsOr, res, ...
    'NumSignals',10, 'Method','fir', 'FIR_Order',512, ...
    'WidthFactor',3, 'TransWidthHz',0.15, 'K',3);

% Visualisation rapide : PSD des sorties
figure('Color','w');
for i = 1:size(S_fft,2)
    subplot(5,2,i); % 10 subplots
    pwelch(S_fft(:,i), hann(2048,'periodic'), 1024, 4096, FsOr);
    title(sprintf('Candidat #%d  fc=%.1f Hz  Δ=%.1f Hz', i, info_fft.fc_Hz(i), info_fft.Delta_Hz(i)));
end
...
for m = 1:M
    accOrigi = S_fft(:,m);
    lags = 1:numel(accOrigi);
    signalEnvelope = getEnv(accOrigi, FsOr, info_fft.fc_Hz(i));% res.best.fc BEST.band(1)-50);  %   extracts the envelope
    [r] = nsdf_mcleod_fast(signalEnvelope, FsOr, 0, 160, numel(signalEnvelope));
    ...
    metrics(m).acf   = r;
    metrics(m).lags  = lags/FsOr;           % en secondes
...
%      accOrigi=array((sizeSlide*(slide_idx-1))+1:(sizeSlide*slide_idx));
    ...
    signalEnvelope = getEnv(acc, Fs, res.best.band(1));% res.best.fc BEST.band(1)-50);  %   extracts the envelope
    [AcfIn] = nsdf_mcleod_fast(signalEnvelope, Fs, 0, 160, numel(signalEnvelope));
    M = acf_quality_metrics(AcfIn);
    Qacf(slide_idx)  = M.Qacf; % should be minimu
    Qspec(slide_idx) = M.Qspec; % sould be max
    Qent(slide_idx)  = M.Qent; %should be max
    fprintf('candidates=%d M.Qacf=%d M.Qspec=%d M.Qent=%d\n',slide_idx,M.Qacf,M.Qspec,M.Qent);
