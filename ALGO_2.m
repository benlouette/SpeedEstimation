...
for slide_idx = 1:n
    ...
    % step 1 peak best candidat
    res = pick_best_carrier_comb(accOrigi, Fs, ISPLOTING);   % x: signal colonne

    
    % step 2 analyse Best candidate
    [f0 , idxBestScore] = analyse_ACF_From_Candidate( accOrigi,Fs, 10, res.candidates.fc_Hz ,fmax,ISPLOTING);
    
    %%
    %     accFir = S_fft(:,idxBestScore);
    %     signalEnvelope = getEnv(accFir, Fs, res.candidates.fc_Hz(idxBestScore));% res.best.fc BEST.band(1)-50);  %   extracts the envelope
    signalEnvelope = getEnvCut(accOrigi, Fs, f0, fmax);
    ...
    %% Estimationcepstrum of raw data
    [ c_q, q_axis,candf_Cepstrum] = coarse_cepstrum_f0(accOrigi, Fs, [fmin fmax], 'Win','hann','Dither',1e-3);
    ...
    elapsed = toc(t1);
    fprintf('period_from_acf_median_robust %.3f s\n', elapsed);
    t1 = tic;
...
function [f0 , idxBestScore] = analyse_ACF_From_Candidate( accOrigi,Fs, nbrCandidate, candidates_fc,fmax,ISPLOTING)
    M = nbrCandidate;
    ...
    for m = 1:M
        %         accFir = S_fft(:,m);
        lags = 0:numel(accOrigi)-1;
        signalEnvelope = getEnvCut(accOrigi, Fs,  candidates_fc(m),fmax);
        [r] = nsdf_mcleod_fast(signalEnvelope, Fs, 0, fmax, numel(signalEnvelope));
        if ISPLOTING
                figure;
                plot(r);
                title(num2str(m));
...
sgtitle('ACF Quality Metrics');
        ...
        xlabel('Candidate'); ylabel('Score ACF (0..1)');
        title('Ranking by ACF quality (higher = better)');
        ...
        xlabel('Candidate (column S index)'); ylabel('Metric score (0..1)');
        title('Comparison of ACF metrics – TopK');
        ...
        xlabel('Lag (s)'); ylabel('ACF (norm.)'); title(sprintf('ACF – Candidate #%d', c1));
        ...
        title('ACF spectrum (baseline-removed)');
    end
end
...
function [ c_q, q_axis,candf_Cepstrum] = coarse_cepstrum_f0(x, Fs, frange, varargin)
...
% conf = max(0, min(1, (peakval - baseline) / (abs(peakval)+eps)));
    c_q=c_q(1:min(N,Fs/frange(1)));
    q_axis=q_axis(1:min(N,Fs/frange(1)));
    f_axis = 1 ./ q_axis;
    mask = (f_axis > frange(2));
    c_q(mask) = 0;
    [~,idx]=max(c_q);
...
function BEST = pick_best_carrier_comb(x, Fs, ISPLOTING)
...
if(ISPLOTING)
    plot_pick_best_comb(BEST, Opt, 10);
end
end
...
title('Panorama : PSD (Welch) and candidats sorted');
...
xlabel('\Delta (Hz)');
title(sprintf('Δ-Scan around %.2f Hz (best Δ = %.2f Hz)', B.fc, B.Delta));
...
function M = acf_quality_metrics2(r, Opt)
% Qualité d'une ACF (sans Qacf), ROBUSTE avec Qharm2 (peigne ACF + UsedK + ZC).
% Sortie M: struct avec Qspec, Qent, Qsfm, Qpksep, Qprom, Qharm2, Tharm (+ debug)

if nargin<2, Opt = struct; end
DEF = struct( ...
    'IgnoreDC', true, 'BaselineFrac', 0.01, 'MedianLenMin', 5, ...
    'Taper', true, 'Win', 'tukey', 'TukeyAlpha', 0.1, ...
    'MinPeakDistBins', 2, ...
    ... % ==== options "peigne ACF" ====
    'Harm_K', 8, ...             % nb d'ordres kT testés
    'Harm_TolRel', 0.06, ...     % fenêtre ±TolRel*T autour de kT
    'Harm_ThrAbs', 0.15, ...     % seuil ACF pour compter un pic "présent"
    'Harm_wMode','1/k', ...      % pondération des ordres
    'Harm_PrefShort', true, ...
    'Harm_KeepFrac', 0.12, ...   % sous-multiple accepté si >= (1-keep)*score
    'Harm_UseCMNDF', true, ...
    'Harm_CMNDFtol', 0.06, ...   % si d'(T/2) ≲ d'(T)+tol -> promeut T/2
    'Harm_TminBins', 16, ...     % borne inf sur T (échantillons)
    'Harm_TmaxFrac', 0.33, ...   % borne sup T ≈ N*frac
    ... % ==== renforts UsedK & zero-crossings ====
    'Harm_BetaUsedK', 0.9, ...   % boost (UsedK/K)^beta
...
r = r(:);
r = r - mean(r,'omitnan');
if ~isempty(r) && r(1) ~= 0, r = r / max(r(1), eps);
else,                     r = r / max(abs(r)+eps);
end
N = numel(r); if N < 16, M = empty_out_metrics(); return; end

if Opt.Taper
    switch lower(Opt.Win)
        case 'hann',  w = hann(N);
        otherwise,    w = tukeywin(N, Opt.TukeyAlpha);
    end
    r = r .* w;
end

% --------- spectre (pour Qspec/Qent/Qsfm/Qpksep/Qprom) ---------
Rspec = abs(fft(r)).^2; Rspec = Rspec(1:floor(N/2));
if Opt.IgnoreDC && ~isempty(Rspec)
    if numel(Rspec) >= 4, Rspec(1) = median(Rspec(2:min(8,end)));
    else,                 Rspec(1) = 0;
    end
end
...
if Ssum <= 0
    M = empty_out_metrics();
    M.debug.Rspec = Rspec; M.debug.Rs = Rs; M.debug.baselineLen = Lb;
    return
end

Qspec = max(Rs) / Ssum;

ps = Rs / Ssum; ps = ps(ps>0);
H  = -sum(ps .* log(ps));
Hmax = log(max(numel(ps),1));
Qent = 1 - H / max(Hmax, eps);

AF = mean(Rs); GF = exp(mean(log(Rs + eps)));
Qsfm = 1 - min(GF/AF, 1);
...
% --------- Qharm2 : peigne ACF + UsedK + pénalité ZC ---------
% 1) candidats T depuis les pics de r (hors voisinage de zéro)
Tmin = max(Opt.Harm_TminBins, 2);
Tmax = min(N-1, round(Opt.Harm_TmaxFrac * (N-1)));

[pv, pl] = findpeaks(r, 'MinPeakDistance', max(8, round(0.5*Tmin)));
mask = (pl>=Tmin & pl<=Tmax); pl = pl(mask); pv = pv(mask);

[~,ord] = sort(pv,'descend'); pl = pl(ord); pv = pv(ord);
pl = pl(1:min(8,numel(pl)));
Tcands = unique([pl(:); round(pl(:)/2); round(pl(:)/3)]);
Tcands = Tcands(Tcands>=Tmin & Tcands<=Tmax);
if isempty(Tcands), Tcands = (Tmin:round(0.02*(Tmax-Tmin)):Tmax).'; end

% pondérations des ordres
if strcmpi(Opt.Harm_wMode,'equal'), w = ones(Opt.Harm_K,1);
else, w = 1./(1:Opt.Harm_K).';
end
w = w / sum(w);

% score de base + renforts
nC = numel(Tcands);
S0 = -inf(nC,1); S2 = -inf(nC,1);
Reg = zeros(nC,1); Cov = zeros(nC,1); Used = zeros(nC,1);
ZC  = zeros(nC,1);

for i=1:nC
    T = Tcands(i);
    % base + reg + cov + used
    [S0(i), Reg(i), Cov(i), Used(i), vals, idxs] = local_score_used(r, T, Opt.Harm_K, Opt.Harm_TolRel, Opt.Harm_ThrAbs, w, N);
    % ZC penalty (excess crossings)
    Z = zero_cross_excess(r, T, Opt.Harm_K, 'SmoothLen',Opt.Harm_ZcSmooth, 'MinSeg',Opt.Harm_ZcMinSeg);
    ZC(i) = Z;
    Zexp  = 2 * min(Used(i), Opt.Harm_K);
    exc   = max(0, (Z - Zexp) / max(Zexp, eps));
    Pzc   = exp(-Opt.Harm_AlphaZC * exc);
    boost_used = (max(Used(i),1)/Opt.Harm_K)^Opt.Harm_BetaUsedK;
    S2(i) = S0(i) * boost_used * Pzc;   % *** SCORE RENFORCÉ ***
    % (on garde vals/idxs si tu veux tracer plus tard)
end

% meilleur T brut
[~, ib] = max(S2); T0 = Tcands(ib);
Tbest = T0; Sbest = S2(ib);

% promotion sous-multiples
subList = unique([round(T0/2), round(T0/3)]);
subList = subList(subList>=Tmin & subList<=Tmax);
for t = subList(:).'
    j = find(Tcands==t,1);
    if isempty(j)
        [St,~,~,Used_t] = local_score_used(r, t, Opt.Harm_K, Opt.Harm_TolRel, Opt.Harm_ThrAbs, w, N);
        Zt   = zero_cross_excess(r, t, Opt.Harm_K, 'SmoothLen',Opt.Harm_ZcSmooth, 'MinSeg',Opt.Harm_ZcMinSeg);
        Zexp = 2 * min(Used_t, Opt.Harm_K);
        exc  = max(0, (Zt - Zexp)/max(Zexp,eps));
        Pzc  = exp(-Opt.Harm_AlphaZC * exc);
        boost= (max(Used_t,1)/Opt.Harm_K)^Opt.Harm_BetaUsedK;
        St2  = St * boost * Pzc;
    else
        St2 = S2(j);
    end
    if St2 >= (1 - Opt.Harm_KeepFrac)*Sbest
        if t < Tbest, Tbest = t; Sbest = St2; end
    end
end

% veto CMNDF (optionnel)
if Opt.Harm_UseCMNDF
    T2 = round(Tbest/2);
    if T2>=Tmin
        dp_T  = cmndf_at(r, Tbest);
        dp_T2 = cmndf_at(r, T2);
        if dp_T2 <= dp_T + Opt.Harm_CMNDFtol, Tbest = T2; end
    end
end

% Qharm2 robuste (échelle 0..1)
Smed = median(S2(isfinite(S2)));
Qharm2 = Sbest / max(Sbest + max(Smed, eps), eps);
...
T = round(T);
if T<2 || T>=N, S=-inf; reg=0; cov=0; used=0; vals=[]; idxs=[]; return; end
vals = nan(K,1); idxs = nan(K,1); hits=false(K,1); used=0;
for k=1:K
    pos = k*T; if pos>=N, break; end
    win = max(1, round(tolRel*T));
    i1 = max(2, pos - win); i2 = min(N-1, pos + win);
    [vmax,irel] = max(r(i1:i2));
    idx = i1 + irel - 1;
    vals(k)=vmax; idxs(k)=idx; used=used+1; hits(k) = (vmax>=thrAbs);
end
if used==0, S=-inf; reg=0; cov=0; return; end
cov = sum(hits(1:used))/used;
if used>=3
    dd = diff(idxs(1:used)); reg = 1 - min(1, std(dd)/max(mean(dd),eps));
else, reg = 0;
end
base = sum(w(1:used).*max(vals(1:used),0));
penT = 1 / (1 + 0.25*(T / (N-1)));    % douce pénalité grands T
S = base * (0.5 + 0.5*reg) * (0.5 + 0.5*cov) * penT;
end

function Z = zero_cross_excess(r, T, K, varargin)
p = inputParser;
addParameter(p,'SmoothLen',[]);
addParameter(p,'MinSeg',[]);
parse(p,varargin{:}); o=p.Results;

N = numel(r);
T = round(T);
i1 = max(2, T); i2 = min(N-1, (K+1)*T);
if i2 <= i1, Z = 0; return; end
seg = r(i1:i2);

if isempty(o.SmoothLen), Ls = max(5, round(0.1*T)); else, Ls = o.SmoothLen; end
Ls = max(3, 2*floor(Ls/2)+1);
base = movmedian(seg, Ls, 'omitnan');
y = seg - base;

if isempty(o.MinSeg), o.MinSeg = max(10, round(0.5*T)); end
if numel(y) < o.MinSeg, Z = 0; return; end

s = sign(y); s(s==0) = 1;
Z = sum(s(1:end-1).*s(2:end) < 0);
end

function dpt = cmndf_at(r, tau)
tau = round(tau); tau = max(1, min(tau, numel(r)-1));
d = 2*(1 - r(2:tau+1));
cum = cumsum(d);
dpt = d(end) * (tau / max(cum(end), eps));
end
