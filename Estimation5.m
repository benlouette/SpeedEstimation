...
array1A=test1.X097_FE_time;
array1B=test1.X097_DE_time;
...
array3A=test3.X099_FE_time;
array3B=test3.X099_DE_time;

test4 = load('Normal Baseline Data 12K/Normal_3_1730_rpm.mat'); % 1725 Rpm
freqRot4 = 1725/60; % tours/min → Hz
array4A=test4.X100_FE_time;
array4B=test4.X100_DE_time;
...
for slide_idx = 1:n
    ...
    [nsdf] = nsdf_mcleod_fast(acc, Fs, 1, 120, 1.0);
...
%     acfHuber = huber(acfDetrended);
    ...
    SH = srh_score(acc, Fs, candf_Cepstrum/2, Kharm_KSRH);
    ST = srh_score(acc, Fs, candf_Cepstrum, Kharm_KSRH);
    ...
    SH = srh_score(acc, Fs, candf_nsdf_pk/2, Kharm_KSRH);
    ST = srh_score(acc, Fs, candf_nsdf_pk, Kharm_KSRH);
    iscorrection_nsdf_pk = false;
    if SH >= ST
        iscorrection_nsdf_pk = true;
        candf_nsdf_pk = candf_nsdf_pk*2;
    end
    ...
    SH = srh_score(acc, Fs, candf_nsdf_pk_mediane/2, Kharm_KSRH);
    ST = srh_score(acc, Fs, candf_nsdf_pk_mediane, Kharm_KSRH);
    if SH >= ST
    ...
    iscorrectionYin = false;
    SH = srh_score(acc, Fs, candf_yin/2, Kharm_KSRH);
    ST = srh_score(acc, Fs, candf_yin, Kharm_KSRH);
    if SH >= ST
        iscorrectionYin = true;
        candf_yin = candf_yin*2;
    end
    ...
    iscorrectionLog = false;
    SH = srh_score(acc, Fs, candf_comb/2, Kharm_KSRH);
    ST = srh_score(acc, Fs, candf_comb, Kharm_KSRH);
    if SH >= ST
        iscorrectionLog = true;
        candf_comb = candf_comb*2;
    end
    ...
    SH = srh_score(acc, Fs, candf_hps/2, Kharm_KSRH);
    ST = srh_score(acc, Fs, candf_hps, Kharm_KSRH);
    if SH >= ST
...
%         'LabelOrientation','horizontal','LabelVerticalAlignment','bottom');
    ...
    for i=1:numel(vals)
        if isfinite(vals(i)) 
            ...
            if(iscorrectionYin && strcmp( names(i), 'YIN') )
                text(xt(i), vals(i)+0.02*max(vals), ...
                    sprintf('%.2f Hz\n%s',vals(i),"CorSRH"), ...
                    'HorizontalAlignment','center','VerticalAlignment','bottom','FontSize',9);
            elseif(iscorrection_nsdf_pk && strcmp( names(i), 'peak') )
                text(xt(i), vals(i)+0.02*max(vals), ...
                    sprintf('%.2f Hz\n%s',vals(i),"CorSRH"), ...
                    'HorizontalAlignment','center','VerticalAlignment','bottom','FontSize',9);
            elseif(iscorrectionhps && strcmp( names(i), 'HPS') )
                text(xt(i), vals(i)+0.02*max(vals), ...
                    sprintf('%.2f Hz\n%s',vals(i),"CorSRH"), ...
                    'HorizontalAlignment','center','VerticalAlignment','bottom','FontSize',9);
                
            elseif(iscorrectionCepstrum && strcmp( names(i), 'Cpst') )
                text(xt(i), vals(i)+0.02*max(vals), ...
                    sprintf('%.2f Hz\n%s',vals(i),"CorSRH"), ...
                    'HorizontalAlignment','center','VerticalAlignment','bottom','FontSize',9);
                
            elseif(iscorrectionMediane && strcmp( names(i), 'MEDIANE') )
                text(xt(i), vals(i)+0.02*max(vals), ...
                    sprintf('%.2f Hz\n%s',vals(i),"CorSRH"), ...
                    'HorizontalAlignment','center','VerticalAlignment','bottom','FontSize',9);
            else
