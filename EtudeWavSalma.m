...
[array, FsOr] = audioread('C:\Users\GJ2640\Downloads\BB1-5596 D_1111_MASK_001_2.wav');  

% signal : matrice contenant les échantillons audio
% Fs     : fréquence d'échantillonnage (Hz)

% Créer un vecteur temps associé
N = length(array);             % nombre d'échantillons
t = (0:N-1) / FsOr;               % vecteur temps en secondes
sound(array,FsOr);
sizeSlide=min(8192*2,numel(array));
n=floor(numel(array)/sizeSlide);
% 
% --- Exemple d’appel ---
fc_list = 200:100:3000;               % Hz
Opt = struct();
...
%     [result,acf] = defineBestRange(accOrigi,FsOr,fc_list(f),2);
%     figure;plot(acf);title(sprintf('%d',f));
%     scores(f) = result;
% end
% [~,idx]=max(scores);
% bestfc = fc_list(idx);
...
for slide_idx = 1:n
    ...
    [AcfIn] = nsdf_mcleod_fast(signalEnvelope, Fs, 0, 50, numel(signalEnvelope));
...
function [result, AcfIn] = defineBestRange(accOrigi,FsOr,f_cut,dec)
