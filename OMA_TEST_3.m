...
for slide_idx = idxBestScore:idxBestScore
...
function [Q, fL, fR, f0] = q_from_psd_db(F, Pdb, fc_guess)
% F: fréquences (Hz), Pdb: PSD en dB (même que Pdbsr), fc_guess ~ fcand(i)
% Retourne Q = f0/BW, et les croisements -3dB (fL,fR) + pic raffiné f0.

% --- recentrer sur le vrai max local proche de fc_guess
[~, i0] = min(abs(F - fc_guess));
w = 3;
iA = max(2, i0-w);
...
% (option) raffine la position du pic par parabole en dB
p_1 = Pdb(i0-1); p0 = Pdb(i0); p1 = Pdb(i0+1);
den = max((p_1 - 2*p0 + p1), eps);
delta = 0.5*(p_1 - p1)/den;              % déplacement sub-bin [-0.5,0.5]
delta = max(-0.5, min(0.5, delta));
f0 = F(i0) + delta*(F(i0+1)-F(i0));

pk_db = p0 - 0.25*(p_1 - p1)*delta;      % hauteur parabolique (option)
th = pk_db - 3;                           % seuil -3 dB

% --- croisement gauche
iL = i0; 
while iL > 1 && Pdb(iL) > th, iL = iL - 1; end
if iL == i0 || iL == 1
    fL = F(max(1,iL));
else
    % interpolation linéaire en dB entre (iL, iL+1)
    p1L = Pdb(iL);   f1L = F(iL);
    p2L = Pdb(iL+1); f2L = F(iL+1);
    alpha = (th - p1L) / max(p2L - p1L, eps);
    alpha = max(0,min(1,alpha));
    fL = f1L + alpha*(f2L - f1L);
end

% --- croisement droit
iR = i0;
while iR < numel(Pdb) && Pdb(iR) > th, iR = iR + 1; end
if iR == i0 || iR == numel(Pdb)
    fR = F(min(numel(F),iR));
else
    p1R = Pdb(iR-1); f1R = F(iR-1);
    p2R = Pdb(iR);   f2R = F(iR);
    alpha = (th - p1R) / max(p2R - p1R, eps);
    alpha = max(0,min(1,alpha));
    fR = f1R + alpha*(f2R - f1R);
end

BW = max(2*mean(diff(F)), fR - fL);   % garde-fou: ≥ ~2 bins
Q  = f0 / max(BW, 1e-6);
end

function debug_plot_Q(F, Pdb, fc_guess)
[Q, fL, fR, f0] = q_from_psd_db(F, Pdb, fc_guess);

% seuil -3 dB du pic trouvé
[~, i0] = min(abs(F - f0));
p0 = Pdb(i0);
th = p0 - 3;

figure; 
plot(F, Pdb); hold on; grid on;
xline(fL, '--'); xline(fR, '--'); 
xline(f0, '-.');
yline(th, ':');
title(sprintf('Q=%.2f, f0=%.2f Hz, BW=%.2f Hz', Q, f0, fR-fL));
xlabel('Hz'); ylabel('PSD (dB)');
xlim([max(min(F), f0-10*(f0-fL)), min(max(F), f0+10*(fR-f0))]); % zoom large auto
end
